<script>
	// Found on Codepen: https://codepen.io/Ducke/pen/zBwGgm while googling "html futuristic button"
	// _no_ idea about licensing -- but I feel it's pretty innocent (for now)
	// TODO: Give credit somewhere
	...
	switch(color) {
		case "red" :
			outerCls = "holo-btn-reb", innerCls = "cta-x";
			break;

		case "green" :
			outerCls = "holo-btn", innerCls = "cta-e";
			break;

		case "blue" :
		default :
			outerCls = "holo-btn-fed", innerCls = "cta-d";
			break;
	}
	...
	<a class={outerCls} href="#void" on:click|preventDefault={clicked}>
		<span class={innerCls}>{label}</span>
		{#if !pressed}
			<span class='skew top'></span>
		{:else}
			<span class='skewDummy top'></span>
		{/if}
